...
export const isNestRuleWithinRule = (node) =>
  node.type === "atrule" && node.name === "nest" &&
  Object(node.parent).type === "rule" && comma(node.params).every((selector) =>
    selector.split("&").length >= 2 && validSelector.test(selector)
  );
