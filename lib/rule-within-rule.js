...
export default function transformRuleWithinRule(node) {
  ...
  const areSameRule = (node.type === "rule" && parent.type === "rule" &&
    node.selector === parent.selector) ||
    (node.type === "atrule" && parent.type === "atrule" &&
...
export const isRuleWithinRule = (node) =>
  node.type === "rule" && Object(node.parent).type === "rule" &&
  node.selectors.every((selector) =>
    selector.trim().lastIndexOf("&") === 0 && validSelector.test(selector)
  );
